...
def seed_database(db: Session, only_if_empty: bool = False) -> int:
    if only_if_empty and db.query(Prompt).count() > 0:
        return 0
...
def main() -> None:
    ...
    try:
        ...
        print(f"Seeded Chaukidar database. Added {seeded} prompts.")
    finally:
        db.close()
